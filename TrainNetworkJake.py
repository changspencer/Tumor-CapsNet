...
'''
primaryCaps = PrimaryCap(inputs=conv1, dim_capsule=1, n_channels=256, kernel_size=9, strides=2, padding='valid')
'''
...
'''
capLayer1 = CapsuleLayer(num_capsule=32, dim_capsule=8, routings=3, name="SecondLayer")(primaryCaps)
'''
...
'''
capLayer2 = CapsuleLayer(num_capsule=3, dim_capsule=16, routings=3, name="ThirdLayer")(capLayer1)
...
masked_by_y = Mask()([capLayer2, y])  # The true label is used to mask the output of capsule layer. For training
masked = Mask()(capLayer2)  # Mask using the capsule with maximal length. For prediction
